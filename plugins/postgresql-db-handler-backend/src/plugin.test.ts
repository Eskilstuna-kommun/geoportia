...
import { postgresqlDbHandlerPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [postgresqlDbHandlerPlugin],
    });

    await request(server).get('/api/postgresql-db-handler/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/postgresql-db-handler/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/postgresql-db-handler/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/postgresql-db-handler/todos/${createdTodoItem.id}`)
